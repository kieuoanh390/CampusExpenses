...
DBHelper extends SQLiteOpenHelper {
    ...
    public int checkUserOrEmailExist(String username, String email) {
        SQLiteDatabase db = this.getReadableDatabase();
        String queryUsername = "SELECT id FROM users WHERE username = ?";
        String queryEmail = "SELECT id FROM users WHERE email = ?";

        boolean usernameExists = false;
        boolean emailExists = false;

        Cursor cursorUsername = db.rawQuery(queryUsername, new String[]{username});
        if (cursorUsername.getCount() > 0) {
            usernameExists = true;
        }
        cursorUsername.close();

        Cursor cursorEmail = db.rawQuery(queryEmail, new String[]{email});
        ...
        // Return 1 for username, 2 for email, 3 for both
        if (usernameExists && emailExists) return 3;
